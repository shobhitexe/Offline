...
const sheetVariants = cva(
  "ui-fixed ui-z-50 ui-gap-4 ui-bg-[#00000F] ui-p-2 ui-shadow-lg ui-transition ui-ease-in-out data-[state=open]:ui-animate-in data-[state=closed]:ui-animate-out data-[state=closed]:ui-duration-300 data-[state=open]:ui-duration-500",
  {
    variants: {
      side: {
        top: "ui-inset-x-0 ui-top-0 ui-border-b data-[state=closed]:ui-slide-out-to-top data-[state=open]:ui-slide-in-from-top",
        bottom:
          "ui-inset-x-0 ui-bottom-0 ui-border-ts data-[state=closed]:ui-slide-out-to-bottom data-[state=open]:ui-slide-in-from-bottom",
        left: "ui-inset-y-0 ui-left-0 ui-h-full ss:ui-w-1/2 ui-w-3/4 data-[state=closed]:ui-slide-out-to-left data-[state=open]:ui-slide-in-from-left sm:ui-max-w-sm",
        right:
...
>(
  (
    { side = "right", stroke = "black", className, children, ...props },
    ref
  ) => (
    <SheetPortal>
      <SheetOverlay />
      ...
      >
        ...
        <SheetPrimitive.Close className="ui-absolute ui-right-4 ui-top-4 ui-rounded-sm ui-opacity-70 ui-ring-offset-background ui-transition-opacity hover:ui-opacity-100 focus:ui-outline-none focus:ui-ring-2 focus:ui-ring-ring focus:ui-ring-offset-2 disabled:ui-pointer-events-none data-[state=open]:ui-bg-secondary">
          <X className="ui-h-5 ui-w-5" stroke={stroke} />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("ui-text-lg ui-font-semibold ui-text-[#FFFFFF]", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("ui-text-sm ui-text-muted-[#FFFFFF]", className)}
    {...props}
  />
));
