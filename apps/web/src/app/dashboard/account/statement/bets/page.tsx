...
import { DashboardHeading, BetStatementColumns } from "@/components";
...
export default function AccountStatement() {
  ...
  const { data } = useSWR<{ data: [] }>(
    `${BackendURL}/api/v1/user/statement/bets?id=${session.data?.user.id}&from=${date?.from?.toISOString()}&to=${date?.to?.toISOString()}&gameType=${gameType}&marketType=${marketType}`,
    fetcher
  );

  return (
    <div className="w-[95%] mx-auto">
      <DashboardHeading heading={"Bet Statement"} />
